import { Component, OnInit, QueryList, ViewChild, ViewChildren, ElementRef, OnDestroy } from '@angular/core';
...
@Component({
  selector: 'app-nuevopedidoconmodal',
  templateUrl: './nuevopedidoconmodal.component.html',
  styleUrls: ['./nuevopedidoconmodal.component.css'],
  providers: [DatePipe],
  
})

export class NuevopedidoConModalComponent implements OnInit, OnDestroy {
  ...
  // Modal state
  public modalAgregarProductoAbierto = false;
  ...
  // Inicializa el form del modal (agrega producto_descripcion)
  this.modalForm = this.fb.group({
    producto: ['', Validators.required],
    producto_descripcion: [''],
    cantidad: [1, [Validators.required, Validators.min(1)]],
    precio: [0, [Validators.required, Validators.min(0)]],
    bonificacion: [0, [Validators.min(0), Validators.max(100)]],
    bonificacion4: [0, [Validators.min(0), Validators.max(10)]],
    total: [{value: 0, disabled: true}]
  });

  // Calcula el total automáticamente
  this.modalSubs.push(
    this.modalForm.valueChanges.subscribe(val => {
      const cantidad = val.cantidad || 0;
      const precio = val.precio || 0;
      ...
      const total = this._pagesService.recalculoTotalItem(cantidad, precio, bonificaciones);
      this.modalForm.get('total').setValue(total, { emitEvent: false });
    })
  );
  }
  ...
  validoItemDeshabilitado(numeroItem:number):boolean{
    const item = this.items && this.items.controls ? this.items.controls[numeroItem] : undefined;
    if (!item) {
      return false;
    }
    if (numeroItem!=0){
  ...
  telefonoRequeridoClientes(){
    ...
    aceptarModalAgregarProducto() {
      if (this.modalForm.valid) {
        // Crea un nuevo FormGroup igual al de newItem()
        const nuevoItem = this.fb.group({
          cantidad: [this.modalForm.value.cantidad, Validators.required],
          producto: [this.modalForm.value.producto, Validators.required],
          producto_descripcion: [''],
          descripcion: [''],
          precio: this.modalForm.value.precio,
          bonificacion: [this.modalForm.value.bonificacion],
          bonificacion1: [0],
          bonificacion2: [0],
          bonificacion3: [0],
          bonificacion4: [this.modalForm.value.bonificacion4],
          pesokg: [0],
          total: this.modalForm.value.total,
        }, {
          validators: [
            this.numeroMayorACero('cantidad'),
            this.controlBonificacion('bonificacion'),
            this.controlBonificacionAdicional('bonificacion4')
          ]
        });
    ...
    ngOnDestroy(): void {
      this.modalSubs.forEach(s => s.unsubscribe());
    }
}
